<script>
        ...
        socket.on('connect', () => {
            if (socket.recovered) {
                ...
                if (localstorage_name) {
                    //localstorage에 name이 있는 유저의 경우 rejoin
    ...
    const display_bubble = (elem, guess, is_self = false) => {
        ...
        if (guess.guess) {
            ...
            if (is_lock) {
                elem.querySelector('.bubble').innerHTML =
                    `<svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" height="0.8rem" width="0.8rem" viewBox="0 0 448 512"><path d="M144 144v48H304V144c0-44.2-35.8-80-80-80s-80 35.8-80 80zM80 192V144C80 64.5 144.5 0 224 0s144 64.5 144 144v48h16c35.3 0 64 28.7 64 64V448c0 35.3-28.7 64-64 64H64c-35.3 0-64-28.7-64-64V256c0-35.3 28.7-64 64-64H80z"/></svg>`;
            } else {
                elem.querySelector('.bubble').innerHTML = guess.guess;
            }
        } else if (guess.text) {
...
{#if step == 0}
    <div class="w-full h-[100dvh] flex flex-col justify-center items-center p-4 bg-zinc-900 text-zinc-100">
        <div class="mb-16 font_test">
            <div class="text-lg">웰컴 투</div>
            <div class="font-bold text-2xl">다함께 꼬맨틀</div>
        </div>
        <div>닉네임 뭐 할거야</div>
        <div class="w-2/3">
            <div class="mb-6">
            ...
            <div class="">
                {#if name}
                    <button on:click={name_submit} class="bg-zinc-800 text-white rounded-lg w-full py-4">
                        입장!🤸
                    </button>
                {/if}
            </div>
...
{:else if step == 1}
    <div class="w-full h-[100dvh] flex flex-col p-4 bg-zinc-900 text-zinc-200">
        <div
            class="absolute top-3 left-3 bg-zinc-700 p-2 rounded-lg text-xs flex justify-center items-center"
            on:click={reset_name}
        >
        ...
        {#if is_admin}
            <div
                class="absolute top-20 right-3 bg-zinc-700 p-2 rounded-lg text-xs flex justify-center items-center"
                on:click={reset_history}
            >
                RESET
            </div>
        {/if}
        <div
            class="absolute top-3 right-3 bg-zinc-700 p-2 rounded-lg text-lg flex justify-center items-center"
        ...
        <!-- options modal -->
        <dialog
            transition:fly={{ duration: 300, y: 500, easing: quintOut }}
        ...
        >
            <div class="flex flex-col justify-start m-4 relative flex-1">
                ...
                <div class="text-lg text-left font-bold mt-5 mb-1 text-zinc-200">설정</div>
                <div class="text-xs text-zinc-300 mb-3">친구가 맞춘 단어를 몇 위까지 볼지 설정할 수 있어요.</div>
                <div class="flex items-start flex-col">
                    <label class="relative inline-flex ms-2 text-sm font-medium text-white items-center my-2">
                        <input
                            type="checkbox"
                            bind:checked={options.show['answer']}
                            on:input={option_change}
                            value="answer"
                            class="sr-only appearance-none peer w-4 h-4 text-blue-600 bg-zinc-600 border-gray-300 rounded"
                        />
                        <div
                            class="w-11 h-6 bg-zinc-300 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-purple-600 mr-5"
                        ></div>
                        정답 보기
                    </label>
                    <label class="relative inline-flex ms-2 text-sm font-medium text-white items-center my-2">
                        <input
                            type="checkbox"
                            bind:checked={options.show['50th']}
                            on:input={option_change}
                            value="50th"
                            class="sr-only appearance-none peer w-4 h-4 text-blue-600 bg-zinc-600 border-gray-300 rounded"
                        />
                    ...
                    <label class="relative inline-flex ms-2 text-sm font-medium text-white items-center my-2">
                        <input
                            type="checkbox"
                            bind:checked={options.show['100th']}
                            on:input={option_change}
                            value="100th"
                            class="sr-only appearance-none peer w-4 h-4 text-blue-600 bg-zinc-600 border-gray-300 rounded"
                        />
                    ...
                    <label class="relative inline-flex ms-2 text-sm font-medium text-white items-center my-2">
                        <input
                            type="checkbox"
                            bind:checked={options.show['250th']}
                            on:input={option_change}
                            value="250th"
                            class="sr-only appearance-none peer w-4 h-4 text-blue-600 bg-zinc-600 border-gray-300 rounded"
                        />
                    ...
                    <label class="relative inline-flex ms-2 text-sm font-medium text-white items-center my-2">
                        <input
                            type="checkbox"
                            bind:checked={options.show['500th']}
                            on:input={option_change}
                            value="500th"
                            class="sr-only appearance-none peer w-4 h-4 text-blue-600 bg-zinc-600 border-gray-300 rounded"
                        />
                    ...
                    <label class="relative inline-flex ms-2 text-sm font-medium text-white items-center my-2">
                        <input
                            type="checkbox"
                            bind:checked={options.show['unknown']}
                            on:input={option_change}
                            value="unknown"
                            class="sr-only appearance-none peer w-4 h-4 text-blue-600 bg-zinc-600 border-gray-300 rounded"
                        />
        ...
        <div class="flex flex-col justify-center items-center mb-4 mt-5">
            <div class="text-2xl font-bold mb-4">다함께 꼬맨틀</div>
            <div class="flex w-full">
                <div class={data.today_similarity ? 'w-4/5 text-xs' : 'text-xs'}>
                    ...
                    <div class="text-zinc-400 text-[0.6rem]">
                        <div>정답, 250위까지 단어는 친구가 맞춰도 보이지 않아요.</div>
                        <div>설정을 변경하려면 우측 상단 버튼을 눌러서 변경하세요.</div>
                    </div>
                </div>
                    ...
                    <div class="text-xs text-zinc-300 mb-1 w-1/5">
                        ...
                        <div class="text-left flex flex-col gap-1">
                            <div>
                                1위-
                                <span class="font-bold">
                                    {(data.today_similarity['1st_score'] * 100).toFixed(1)}
                                </span>
                            </div>
                            <div>
                                <span> 10위- </span>
                                <span class="font-bold">
                                    {(data.today_similarity['10th_score'] * 100).toFixed(1)}
                                </span>
                            </div>
                            <div>
                                <span> 1000위- </span>
                                <span class="font-bold">
                                    {(data.today_similarity['1000th_score'] * 100).toFixed(1)}
                                </span>
                            </div>
                        </div>
                    </div>
        ...
        <div class="flex flex-col text-xs">
            <div class="mb-1">접속중인 친구</div>
            <div class="flex flex-wrap user_list">
                {#each user_list as user, index (index)}
                    <!-- {@debug user_list} -->
                    {#if user.socket_id == socket.id}
                        <div
                            class="flex items-center justify-center rounded-full text-xs px-3 py-1 mx-1 bg-zinc-500 my-1 relative"
                            class:crowned-text={top_player.socket_id == user.socket_id || top_player.name == user.name}
                            user_name={user.name}
                        >
                            it's me {user.name}
                            <div class="bubble_container absolute max-w-[110%] text-center text-[0.65rem]">
                                <div class="bubble bottom text-sm px-1 py-0.5 duration-300">진입로</div>
                            </div>
                        </div>
                    {:else}
                        <div
                            class="flex items-center justify-center rounded-full text-xs px-3 py-1 mx-1 bg-zinc-700 my-1 relative"
                            class:crowned-text={top_player.socket_id == user.socket_id || top_player.name == user.name}
                            user_name={user.name}
                        >
                            {user.name}
        ...
        <div class="my-3">
            ...
            <div class="flex">
                <input
                    ...
                    bind:this={guess_input}
                    type="text"
                    on:keydown={keydown}
                    bind:value={guess}
                    class="block w-2/3 p-3 border-t border-l border-b bg-zinc-800 border-zinc-600 text-sm rounded-tl-lg rounded-bl-lg focus:border-none"
                />
                <button
                    on:click={submit_guess}
                    class={waiting
                        ? 'flex w-1/3 justify-center items-center bg-zinc-800 text-zinc-300 rounded-tr-lg rounded-br-lg '
                        : 'flex justify-center items-center  rounded-tr-lg bg-zinc-800 border-r border-t border-b border-zinc-600 rounded-br-lg w-1/3 '}
                    disabled={waiting}
                >
            ...
            {#if current_guess}
                <div class="grid grid-cols-5 gap-2">
                    ...
                    <div>
                        ...
                        {:else if current_guess.sim > data.today_similarity.rest}
                ...
                <div class={guess_text_dynamic_class(guess.rank)}>
                    ...
                    <div class="flex items-center">
                        ...
                        {:else if guess.rank.includes('정답')}
                            ...
                            {/if}
                        {:else if guess.sim > data.today_similarity.rest}
...
<style>
    ...
    .crowned-text {
        ...
        background-image: linear-gradient(rgb(63 63 70), rgb(63 63 70)),
